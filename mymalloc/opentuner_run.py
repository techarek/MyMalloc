...
class MdriverTuner(MeasurementInterface):
  ...
  def run(self, desired_result, input, limit):
    ...
    compile_result = self.call_program(make_cmd, limit = self.args.make_timeout, universal_newlines=True)
    ...
    # Run the command.
    run_result = self.call_program(bin_cmd, limit = self.args.command_timeout, universal_newlines=True)
    ...
    if awsrun:
        ...
        # Don't count result if incorrect
        correct = result.get('correct', correct);
        if correct == 0:
            return Result(accuracy=accuracy, time=time)
